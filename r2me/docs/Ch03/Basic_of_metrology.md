...
!!! note "什么是测量原理"
   所谓测量原理，是指测量过程中用到的物理、化学特性。如长度测量中使用的光干涉原理，在长度测量中不遵守[阿贝误差](https://zhuanlan.zhihu.com/p/597006248)大的测量误差。
...
计量器具是认识被测量的真值大小的设备，包含实物量具（即具有所赋量值，以固定形式复现的测量仪器，如砝码、标尺等）、测量系统、测量设备。

!!! note "需要了解的有关标尺的术语"
     （1）标尺长度：标尺的第一个标记和最后一个标记的长度；  
     （2）标尺间距：标尺长度除以标尺总的间隔数，即两相邻标尺标记之间的距离；  
     （3）标尺间隔（分度值）：两相邻标尺标记的两个值之差；  
