...
3、企业、事业单位制造标准物质，应向国务院计量行政部门申请定级鉴定。  

4、如果要改原始记录，对记录的所有改动应有改动人的**签名或签名缩写**。  

5、内部审核的目的是：验证机构运作和管理体系要求的符合性；  

6、管理评审的目的是：确保管理体系持续得适应性、充分性和有效性。**管理评审**的输出为资源需求、管理体系有效性及其过程的改进、与法律法规有关的检定、校准的改进。  

7、计量机构使用非标方法的确认：1）使用参考标准或标准物质进行校准；2）对影响结果的因素进行系统性评审；3）与其他已确认的方法进行结果比对；4）实验室间比对；5）根据对方法原理的理解以及抽样或检测方法的实践经验，评定结果的测量不确定度；  

8、法定计量检定机构应具有对不符合工作进行控制的政策和程序；a）确定对不符合工作进行管理的责任和权限；b）对不符合工作的严重性进行评价；c）立即进行纠正，同时对不符合工作的可接受性进行判断（**没有预防措施**）；d）必要时，通知顾客并取消工作；e）规定批准恢复工作的职责。  

9、计量安全需要注意的点：1）防盗；2）防火、防爆、3）防水；4）防毒。
...
!!! note "[强制检定的名单](https://scj.sxxz.gov.cn/zljcssyzsfw/jlfw/202107/t20210726_3659463.html)"
...
!!! note "《计量标准考核办法》-计量规章中的原文"
		国家市场监督管理总局组织建立的社会公用计量标准及各省级市场监督管理部门组织建立的各项最高等级的社会公用计量标准，由国家市场监督管理总局主持考核；地（市）、县级市场监督管理部门组织建立的各项最高等级的社会公用计量标准，由上一级市场监督管理部门主持考核；各级地方市场监督管理部门组织建立其他等级的社会公用计量标准，由组织建立计量标准的市场监督管理部门主持考核。
...
4）计量基准不一定是都建立在国家法定计量检定机构，比如一些特殊行业的基准就可以建立在单位内部的计量机构；计量基准的复核周期一般为5年；  
...
12、授权有关部门或单位的**专业性、区域性**计量技术机构，作为法定计量检定机构。  

13、可与国际单位制并用的我国法定计量单位包括：海里 n mile、节 kn。  

14、**真值具有单一量值**是错误的，由于定义本身细节不完善，不存在单一真值，值存在与定义一致的一组真值。  

15、注意量纲前面**没有系数**。  

16、量纲为1的量有哪些？比如什么系数、渗透率、马赫数、平面角和立体角等，线圈的圈数也是量纲为1的量。  

17、量纲相同的量其物理含义不一定相同，如功、力矩和热量量纲都相同，但不是同类量。  

18、测量区间小于等于标称区间。  

19、与漂移示值有关的是**使用时间长短**，我们一般先放置一段时间或预热就是**减少漂移的**影响。
